	...
	public function anadirManifiesto(Request $request)
	{
        ...
        {   
            ...
            if($manifiesto_bdd->count()==0)
            {
            	...
            	$nueva_manifiesto = new ModelManifiesto();
                $nueva_manifiesto->titulo=$request->input('titulo_manifiesto');
                $nueva_manifiesto->fecha=$request->input('fecha_manifiesto');
                $nueva_manifiesto->texto=$request->input('texto_manifiesto');
                if ($request->hasFile('file')) //Si recibimos el file que es de la imagen
    		    {
    		      $file = $request->file('file');
    		      $nombre = $file->getClientOriginalName();
    		      $nueva_manifiesto->imagen=$file->getClientOriginalName();
    		      $path = public_path('imagenes/Manifiestos/');
    		      $file->move($path, $nombre);
    		    }
                
                $nueva_manifiesto->save(); //guardamos en la base de datos
